...
export default function Form() {
  ...
  const modes = [
    {
      ...
      component: (
        <ProductSelect
          ...
          onSizeChange={(n, v) => onSizeChange(n, v)}
          size={userInput.size}
    ...
    },
    {
      ...
      component: (
        <ServiceSelect
          ...
          onServiceSelect={service => handleChange("service", service)}
          add={() => add()}
        />
      )
    }
  ...
  const onSizeChange = (name, value) => {
    let p = userInput.product;
    console.log(p);
  ...
  const add = () => {
    if (!isEmpty(userInput.product)) {
      ...
      for (var i in userInput.supplieridsList) {
        if (userInput.supplieridsList[i] === userInput.product.product.userid) {
          isIn = true;
          return;
        }
      }
        ...
        <div className="row mt-4">
          ...
          <div className="col-md-9 d-flex flex-column">
            <ProductTable
              ...
              onQtyChange={(id, value) => onProductQtyChange(id, value)}
              products={userInput.productsList}
            />

            <ServiceTable
              onQtyChange={(id, value) => onServiceQtyChange(id, value)}
