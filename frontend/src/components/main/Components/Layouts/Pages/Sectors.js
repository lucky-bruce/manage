...
export default function Sectors() {
  ...
  React.useEffect(() => {
    fetch('/mix', {
      ...
      .then((res) => {
        if (!!res.info) {
          let sectorA =  res.info.find( item => item.title === 'Sector A');
          let sectorB =  res.info.find( item => item.title === 'Sector B');
          let sectorC =  res.info.find( item => item.title === 'Sector C');
      ...
      <div className={classes.root} >
        ...
        <TabPanel value={value} index={0}>
          <MDBContainer>
            <MDBRow>
              <MDBCol md={4}>
                { (!!sectorA) ? <img alt='' src={`/${sectorA.image}`} style={{ width: '100%' }} /> :''}
            ...
            <MDBRow><h4>Sector A News</h4></MDBRow>
          </MDBContainer>
          ...
          <div className='team-main'>
            <MDBContainer>
              <MDBRow>
                {
                  news.filter((item) => item.sector === 'A').map((item) => (
        ...
        <TabPanel value={value} index={1}>
          ...
          <MDBContainer>
            <MDBRow>
            <MDBCol md={4}>
                { (!!sectorB) ? <img alt='' src={`/${sectorB.image}`} style={{ width: '100%' }} /> :''}
            ...
            <MDBRow><h4>Sector B News</h4></MDBRow>
          </MDBContainer>
          ...
          <div className='team-main'>
            <MDBContainer>
              <MDBRow>
                {
                  news.filter((item) => item.sector === 'B').map((item) => (
                    <MDBCol key={item._id} md={4} style={{ minHeight: '270px', }}>
                      <NewsItem item={item} />
                    </MDBCol>
                  ))
        ...
        <TabPanel value={value} index={2}>
          <MDBContainer>
            <MDBRow>
            <MDBCol md={4}>
                { (!!sectorC) ? <img alt='' src={`/${sectorC.image}`} style={{ width: '100%' }} /> :''}
              </MDBCol>
              <MDBCol md={8}>
                <p className="mt-2" style={{ textAlign: 'left' }}>
            ...
            <MDBRow><h4>Sector C News</h4></MDBRow>
          </MDBContainer>
          <div className='team-main'>
            <MDBContainer>
              <MDBRow>
                {
                  news.filter((item) => item.sector === 'C').map((item) => (
                    <MDBCol key={item._id} md={4} style={{ minHeight: '270px', }}>
                      <NewsItem item={item} />
                    </MDBCol>
                  ))
